using AutoServiceManager.Application.DTOs.Identity;
using AutoServiceManager.Application.DTOs.Mail;
using AutoServiceManager.Application.DTOs.Settings;
using AutoServiceManager.Application.Enums;
using AutoServiceManager.Application.Exceptions;
using AutoServiceManager.Application.Interfaces;
...
namespace AutoServiceManager.Infrastructure.Identity.Services
{
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _mailService.SendAsync(new MailRequest() { From = "mail@ivanpterov.com", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
